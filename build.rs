...
fn main() {
    ...
    // PRODUCT: the product name is a build parameter rather than a literal, so this fork
    // tracks product.json instead of becoming a second source of truth for the brand.
    // Defaults to upstream's name, so a plain `cargo build` still reproduces upstream.
    let product_name =
        env::var("INNO_UPDATER_PRODUCT_NAME").unwrap_or_else(|_| "Visual Studio Code".to_string());
    let company_name =
        env::var("INNO_UPDATER_COMPANY_NAME").unwrap_or_else(|_| "Microsoft Corporation".to_string());
    println!("cargo:rustc-env=INNO_UPDATER_PRODUCT_NAME={}", product_name);
    println!("cargo:rerun-if-env-changed=INNO_UPDATER_PRODUCT_NAME");
    println!("cargo:rerun-if-env-changed=INNO_UPDATER_COMPANY_NAME");
